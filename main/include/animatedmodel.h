...
public:
	...
	AnimatedModel(string const& path, unsigned int cAnimation = 0, bool gamma = false) : gammaCorrection(gamma)
	{
		this->currentAnimation = cAnimation;
		loadModel(path);
	}
	...
	void SetPose(float time, glm::mat4* out_gBones) {
		...
		for (unsigned int i = 0; i < bones.size(); i++) {
			if (i < 100) {
				...
				out_gBones[i] = bones[i].transformation; // <-- Escribe en el array de salida
...
private:
	...
	void loadModel(string const& path)
	{
		...
		// animationCount = 0; // <-- MOVIDO
		// elapsedTime = 0.0f; // <-- MOVIDO

		// SetPose(0.0f, gBones); // <-- MOVIDO (se hace en la creacin de la instancia)
	}
	...
	void ReadNodeHierarchy(float AnimationTime, const aiNode* pNode, const glm::mat4& ParentTransform)
	{
		...
		if (pAnimation == nullptr) {
			cout << "Error: no valid animation index." << endl;
			return;
		}
